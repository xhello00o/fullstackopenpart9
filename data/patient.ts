...
export const  fulltypedPatientData = patientsData.map(obj => {
    const typedObj = patientEntry(obj);
    return typedObj;
}
    );

export const filtered_data:FilteredPatient[] = fulltypedPatientData.map(person => {
    delete person.ssn;
    return person;
});
